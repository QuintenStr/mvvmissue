...
namespace Odisee.Common.Mediators
{
    ...
    public class Mediator
    {
        ...
        public void Unregister(IMediatorSubscriber subscriber, String message = null)
        {
            foreach (String key in list.Keys)
            {
                if (message == null || key == message)
                {
                    ...
                    {
                        if (Object.Equals(list[key][i], subscriber))
                        {
                            list[key].RemoveAt(i);
                        }
                    }
        ...
        {
            if (list.ContainsKey(message))
                foreach (IMediatorSubscriber reference in list[message])
                    ((IMediatorSubscriber)reference).MediatorNotification(from, message, data);
        }
    }
}
